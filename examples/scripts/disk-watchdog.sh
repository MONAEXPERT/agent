...
#
# A remoteagent recipe: run this from cron (or the dashboard's cron tasks)
# so the agent checks disk health and notifies you before things break.
#
#   crontab -e
#   # every morning at 8:00
#   0 8 * * * /path/to/remoteagent/examples/scripts/disk-watchdog.sh
#
...
if ! command -v remoteagent >/dev/null 2>&1; then
  echo "disk-watchdog: remoteagent not found on PATH" >&2
  exit 1
fi
...
remoteagent chat "Run the disk-health skill: check all volumes. Flag every volume above ${THRESHOLD}% with what it is, how much is free, and one safe cleanup suggestion (propose only, never delete)."
