export class RunStore {
  ...
  checkpoint(id, data){ const r = this.runs.get(id); if(!r) throw new Error(`Run ${id} not found`); const cp = { ts: Date.now(), data }; r.checkpoints.push(cp); r.updatedAt = Date.now(); r.currentCheckpoint = cp; return cp; }
