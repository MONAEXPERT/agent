...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'remoteagent-pkg-'));
...
before(async () => {
  ...
  ({ auditVerify } = await import('@remoteagent/engine'));
});
...
describe('PackageLifecycle install/upgrade/rollback', () => {
  it('installs and confirms a package', () => {
    ...
    const installing = l.install('web-1', 'remoteagent', '2.0.0');
    assert.equal(installing.state, 'installing');
    assert.equal(installing.currentVersion, '2.0.0');
    const installed = l.confirm('web-1', 'remoteagent');
    assert.equal(installed.state, 'installed');
  });

  it('upgrades, fails, and rolls back to the previous version', () => {
    const l = new PackageLifecycle({ storePath: p('b') });
    l.install('web-1', 'remoteagent', '2.0.0');
    l.confirm('web-1', 'remoteagent');

    const upgrading = l.upgrade('web-1', 'remoteagent', '3.0.0');
    assert.equal(upgrading.previousVersion, '2.0.0');
    assert.equal(upgrading.currentVersion, '3.0.0');

    const failed = l.fail('web-1', 'remoteagent', { reason: 'crash on start' });
    assert.equal(failed.state, 'rollback_required');

    const rolled = l.rollback('web-1', 'remoteagent', { reason: 'bad version' });
  ...
  it('rejects invalid transitions and refuses rollback without a previous version', () => {
    ...
    assert.throws(() => l.upgrade('web-1', 'remoteagent', '2.0.0'), /cannot upgrade from absent/);
    l.install('web-1', 'remoteagent', '2.0.0');
    assert.throws(() => l.rollback('web-1', 'remoteagent'), /cannot roll back from installing/);
  });

  it('persists across restart and audits lifecycle transitions', () => {
    const pth = p('d');
    const a = new PackageLifecycle({ storePath: pth });
    a.install('web-1', 'remoteagent', '2.0.0');
    a.confirm('web-1', 'remoteagent');
    a.upgrade('web-1', 'remoteagent', '3.0.0');
    a.fail('web-1', 'remoteagent', { reason: 'x' });
    a.rollback('web-1', 'remoteagent', { reason: 'x' });

    const b = new PackageLifecycle({ storePath: pth });
    assert.equal(b.get('web-1', 'remoteagent').state, 'rolled_back');
