// WebSocket control channel — the device dials OUT to api.remoteagent.online.
  ...
  connect() {
    ...
    this.#ws = new WebSocket(url, {
      headers: {
        ...
        'x-remoteagent-id':  this.#agentId || '',
        'user-agent':        `remoteagent/${DEFAULTS.version}`,
      },
    });
    ...
    this.#ws.on('open', () => {
      ...
      if (CLOUD.platform === 'docker') {
        ...
        this.#sendFlat('register', { name: os.hostname(), model: `remoteagent/${DEFAULTS.version}` });
      } else {
    ...
    this.#ws.on('close', (code) => {
      ...
      if (isTerminalClose(code)) {
        ...
        log.error(`Cloud rejected credentials (code ${code}) — stopping. Run: remoteagent login`);
