// Cloud API client — all LLM reasoning runs remotely on api.remoteagent.online.
// Supports both sngine-based (api.remoteagent.online) and Docker-based platforms.
...
async function apiFetch(path, { apiKey, method = 'POST', body, signal, headers: extraHeaders } = {}) {
  ...
  const headers = {
    ...
    'user-agent':    UA,
    'x-remoteagent':  DEFAULTS.version,
    ...(extraHeaders || {}),
  };
