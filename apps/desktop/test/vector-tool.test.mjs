...
describe('tools/vector', () => {
  it('remembers and searches notes semantically', async () => {
    await vector.run({ action: 'remember', text: 'the deploy server uses port REDACTED_PORT for ssh' });
    ...
    assert.ok(r.hits[0].text.includes('REDACTED_PORT'), `top hit: ${r.hits[0]?.text}`);
