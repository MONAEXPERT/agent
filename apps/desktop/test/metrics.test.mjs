...
describe('metrics server', () => {
  ...
  it('GET /metrics emits prometheus counters and gauges', async () => {
    ...
    assert.match(text, /remoteagent_up 1/);
    assert.match(text, /remoteagent_tasks_total 3/);
    assert.match(text, /remoteagent_tool_calls_total 7/);
    assert.match(text, /remoteagent_errors_total 1/);
    assert.match(text, /remoteagent_connected 1/);
    assert.match(text, /remoteagent_budget_spent_usd 0.0042/);
    assert.match(text, /remoteagent_queue_size 2/);
  });
  ...
  it('live state changes are reflected', async () => {
    ...
    const r = await fetch(`http://127.0.0.1:${PORT}/metrics`);
    const text = await r.text();
    assert.match(text, /remoteagent_connected 0/);
