#!/usr/bin/env node
// remoteagent skills — manage agent skills.
...
switch (cmd) {
  case 'list': {
    ...
    if (!items.length) {
      console.log('No skills installed. Run: remoteagent skills install');
      process.exit(0);
    }
  ...
  case 'enable': {
    if (!arg) { console.error('Usage: remoteagent skills enable <name>'); process.exit(2); }
  ...
  case 'disable': {
    if (!arg) { console.error('Usage: remoteagent skills disable <name>'); process.exit(2); }
    manager.disable(arg);
  ...
  default:
    console.log('Usage: remoteagent skills <list|enable|disable|install>');
    process.exit(2);
}
