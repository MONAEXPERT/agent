...
<!-- GENERATED BY scripts/import-graph.mjs — DO NOT EDIT BETWEEN THE MARKERS -->

Real import edges, read from the source (6 package-level edges):

```mermaid
flowchart LR
    "apps/desktop (device agent)" -->|"113 imports"| "apps/desktop (device agent)"
    "apps/desktop (device agent)" -->|"11 imports"| "packages/engine (device core)"
    "apps/desktop (device agent)" -->|"1 import"| "packages/protocol"
    "packages/control-plane (server library)" -->|"9 imports"| "packages/control-plane (server library)"
    "packages/control-plane (server library)" -->|"10 imports"| "packages/engine (device core)"
    "packages/engine (device core)" -->|"13 imports"| "packages/engine (device core)"
```
