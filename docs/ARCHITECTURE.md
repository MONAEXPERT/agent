...
```mermaid
flowchart LR
    "apps/desktop (device agent)" -->|"115 imports"| "apps/desktop (device agent)"
    "apps/desktop (device agent)" -->|"19 imports"| "packages/engine (device core)"
    "apps/desktop (device agent)" -->|"1 import"| "packages/protocol"
    "packages/control-plane (server library)" -->|"8 imports"| "packages/control-plane (server library)"
    "packages/control-plane (server library)" -->|"11 imports"| "packages/engine (device core)"
    "packages/engine (device core)" -->|"23 imports"| "packages/engine (device core)"
```
