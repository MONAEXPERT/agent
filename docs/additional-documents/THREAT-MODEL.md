...
Scope: the remoteagent client (device daemon) and its communication with the
...
| **Tampering** | Intercept/modify task or results in transit; tamper with the local audit log | TLS 1.2+ everywhere; no plaintext fallback; integrity via Git-tagged releases; the local audit log is **hash-chained and append-only** — any edit breaks the chain and is detected by `remoteagent audit verify` |
...
| **Elevation of privilege** | Prompt injection escalates a tool call | **Local policy is authoritative** — loaded from disk at startup, the cloud can never widen it (allow/deny/confirm per tool, presets). Shell executes argv arrays with a realpath-resolved allowlist (chains and pipes re-check every segment — pipe-to-shell is structurally impossible); files are workspace-confined with symlink/TOCTOU guards; deletes go to trash |
| **Elevation of privilege** | Cloud → device privilege extension (a compromised control plane pushes wider capabilities) | **Signed capability grants with owner ceiling** — nine-stage verification (signature against pinned keys, device/tenant/run binding, TTL, fresh 2FA, nonce replay), intersection with the local ceiling wins, `security: "locked"` short-circuits everything. Countermeasure: `packages/engine/src/capability-grant.js`; tests: `packages/engine/test/attacks-grant.test.mjs`, `capability-grant.test.mjs` |
| **Elevation of privilege** | Plugin import as code execution (a malicious tool package runs its top-level code at load) | **Supply-chain gate** — discovery confined to the install directory + `MONA_TOOL_PATH` (never `process.cwd()`); with keys pinned in `policy.json` a package loads ONLY when its signed manifest verifies and every declared capability is granted — no manifest, no signature, no import. Countermeasure: `apps/desktop/src/tools/registry.js` (`supplyChainGate`); tests: `apps/desktop/test/plugin-tool.test.mjs` (unsigned package must never be imported), `packages/engine/test/plugin-manifest.test.mjs` |
...
7. Audit log tampering → hash chain breaks; `remoteagent audit verify` reports
   the exact entry.
