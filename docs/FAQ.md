...
Yes — MIT licensed, free forever. The [remoteagent.online](https://api.remoteagent.online)
cloud has a free tier.

## Does remoteagent need an API key?
...
```bash
remoteagent provider set anthropic     # or: openai / ollama
...
Yes — with a local provider. `remoteagent provider set ollama` +
...
## Can I run remoteagent on a server / Raspberry Pi?

Yes — any Node.js 20+ machine. Headless mode: `remoteagent start` (or a
...
old. Check `remoteagent start` is running and that the device has HTTPS
egress to api.remoteagent.online.
...
```bash
remoteagent policy preset strict      # read-only (no shell/net/browser)
remoteagent policy preset standard    # shell & browser need approval
remoteagent policy preset permissive  # everything allowed (default)
remoteagent policy explain <tool> …   # why a call is allowed/denied
...
Bugs: [GitHub issues](https://github.com/remoteagent-online/remoteagent/issues).
